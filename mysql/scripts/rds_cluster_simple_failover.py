...
while True:
    try:
        ...
        # Query status
        sql_command = "SELECT @@read_only, @@server_id, @@hostname;"

        # Run the query
        with conn.cursor() as cursor:
            cursor.execute(sql_command)
            (is_reader, server_id, hostname) = cursor.fetchone()
            if is_reader > 0:
                server_role = "reader"
            else:
                server_role = "writer"
        ...
        if failover_detected:
            if is_reader > 0:
                ...
                # Display success
                print("[SUCCESS]", "%s: failover completed, took: ~ %d sec., connected to %s (%s, %s)" % (time.strftime('%H:%M:%S %Z'), (failover_end_time - failover_start_time), server_id, server_role, hostname))
        else:
            if is_reader > 0:
                # Detect failover
                ...
                # Display info
                print("[INFO]", "%s: connected to %s (%s, %s)" % (time.strftime('%H:%M:%S %Z'), server_id, server_role, hostname))
